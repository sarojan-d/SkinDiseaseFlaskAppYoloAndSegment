...
def plot_results(start=0, stop=0, bucket='', id=(), labels=(), save_dir=''):
    ...
    Args:
        ...
        stop (int): Stop epoch.
        bucket (str): Bucket for cloud storage (if used).
        id (tuple): IDs for cloud storage files (if used).
        labels (tuple): Labels for curves.
    ...
    for metric in metrics:
        train_idx, val_idx = indices[metric]  # train and val indices for the metric
        ...
        for fi, f in enumerate(files):
            try:
                ...
                train_y = results[train_idx, x]
                val_y = results[val_idx, x]
                train_y[train_y == 0] = np.nan  # Ignore zero loss values
                val_y[val_y == 0] = np.nan
